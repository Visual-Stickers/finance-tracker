...
async function renderDashboard() {
  const [months, loansRaw] = await Promise.all([
    Storage.getMonths(),
    Storage.getLoans()
  ]);
  ...
  renderTasks(months);
  renderInsights(months, loans, balance);
  renderLoanProgress(loans);
...
function renderInsights(months, loans, balance) {
  const mk = Utils.monthKey(new Date());
  const m = months[mk];
  const insights = [];
...
function renderIncomeExpenseChart(months) {
  const sorted = Object.values(months).sort((a, b) => a.key > b.key ? 1 : -1).slice(0, 8);
  const labels   = sorted.map(m => (m.label || m.key).split(' ')[0]);
  const incomes  = sorted.map(m => m.income || 0);
  const expenses = sorted.map(m => (m.expenseItems || []).reduce((s, e) => s + (e.planned || 0), 0));
...
function renderCashflowChart(months) {
  const sorted   = Object.values(months).sort((a, b) => a.key > b.key ? 1 : -1).slice(0, 10);
  const labels   = sorted.map(m => (m.label || m.key).split(' ')[0]);
...
function renderExpenseBreakdown(months) {
  const mk = Utils.monthKey(new Date());
  const m  = months[mk] || Object.values(months)[0];
  if (!m) return;

  const items  = (m.expenseItems || []).filter(e => (e.planned || 0) > 0);
  const labels = items.map(e => e.name);
  const data   = items.map(e => e.planned);
