...
const DB = {
  ...
  async getMonths(userId) {
    const sb = getSB();
    const [{ data: mRows }, { data: eRows }] = await Promise.all([
    ...
    for (const m of (mRows || [])) {
      months[m.month_key] = {
        ...
        incomeDate: m.income_date, incomeNote: m.income_note,
        balance: m.balance, expenses: [], extraIncomes: []
      };
    }
    for (const e of (eRows || [])) {
      if (!months[e.month_key]) continue;
      const item = { id: e.id, name: e.name, planned: e.planned, actual: e.actual, status: e.status, date: e.date, note: e.note, sort_order: e.sort_order };
  ...
  async saveMonthIncome(userId, monthKey, fields) {
    const sb = getSB();
    const patch = {};
    if (fields.income       !== undefined) patch.income        = fields.income;
    if (fields.incomeActual !== undefined) patch.income_actual = fields.incomeActual;
  ...
  async addExpense(userId, monthKey, { name, planned, note, isIncome }) {
    ...
    const { data: existing } = await sb.from('month_expenses').select('sort_order')
      .eq('user_id', userId).eq('month_key', monthKey).eq('is_income', !!isIncome)
  ...
  async updateExpense(userId, expId, fields) {
    const sb = getSB();
    const patch = {};
    if (fields.actual   !== undefined) patch.actual   = fields.actual;
    if (fields.status   !== undefined) patch.status   = fields.status;
    if (fields.date     !== undefined) patch.date     = fields.date || null;
    if (fields.note     !== undefined) patch.note     = fields.note;
    if (fields.planned  !== undefined) patch.planned  = fields.planned;
  ...
  async createMonth(userId, monthKey, income) {
    ...
    if (error && error.code !== '23505') { console.error(error); return false; }
    return true;
  },
  ...
  async getLoans(userId) {
    ...
    return (data || []).map(r => ({
      ...
      interestRate: r.interest_rate, tenure: r.tenure,
      emisPaid: r.emis_paid, emisRemaining: r.emis_remaining,
  ...
  async saveLoan(userId, loan) {
    ...
    const payload = {
      ...
      tenure: Number(loan.tenure) || 0, emis_paid: Number(loan.emisPaid) || 0,
  ...
  async getCards(userId) {
    ...
    return (data || []).map(r => ({
      ...
      limit: r.limit_amount, outstanding: r.outstanding,
      dueDate: r.due_date, color: r.color || 'blue'
    }));
  },
  ...
  async saveCard(userId, card) {
    ...
    const payload = {
      ...
      due_date: card.dueDate || null, color: card.color || 'blue'
    };
    if (card.id) {
