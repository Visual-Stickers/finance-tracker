...
const Storage = (() => {
  ...
  async function getMonths() {
    ...
    // Attach expenses and additional income sources (sort_order < 0 = income)
    (expRows || []).forEach(e => {
      if (months[e.month_key]) {
        ...
        if (e.sort_order < 0) {
          months[e.month_key].additionalIncomes.push(item);
        } else {
          months[e.month_key].expenseItems.push(item);
        }
      }
    });
