    ...
    return `
      <div class="ie-item">
        ...
        <div class="ie-item-action" style="display:flex;gap:4px">
          ${a.status !== 'received'
            ? `<button class="btn btn-success btn-sm" onclick="markAdditionalIncomeReceived('${a.id}','${a.planned}')"><i class="fa-solid fa-check"></i> Mark</button>`
            : `<button class="btn btn-ghost btn-sm" onclick="markAdditionalIncomeReceived('${a.id}','${a.actual||a.planned}')"><i class="fa-solid fa-pen"></i></button>`
          }
...
async function markAdditionalIncomeReceived(id, defaultAmount) {
  const amount = prompt('Enter received amount:', defaultAmount);
  if (amount === null) return;
  const actual = parseFloat(amount);
  if (isNaN(actual)) { Utils.showToast('Invalid amount', 'error'); return; }
  await Storage.saveExpenseItem(id, {
    actual,
    status: 'received',
