    ...
    <div class="loan-card animate-in" id="lcard_${l.id}">
      ...
      <div class="loan-card-body">
        <div class="loan-stats">
          ...
          <div class="loan-stat-item"><div class="loan-stat-label">EMIs Paid</div><div class="loan-stat-value">${l.emisPaid} / ${l.tenure}</div></div>
          <div class="loan-stat-item"><div class="loan-stat-label">Remaining</div><div class="loan-stat-value">${l.emisRemaining} EMIs</div></div>
      ...
      <div class="loan-card-foot">
        ...
        <button class="btn btn-primary btn-sm" onclick="openEmiModal('${l._supabaseId || l.id}')">
...
function openEmiModal(loanId) {
  const loan = _loans.find(l => (l._supabaseId || l.id) === loanId);
  if (!loan) return;
...
async function saveEmiPayment() {
  const loan = _loans.find(l => (l._supabaseId || l.id) === currentLoanId);
  if (!loan) return;
  ...
  loan.outstanding = Math.max(0, loan.outstanding - (amount * 0.85));
  loan.emisPaid    += 1;
...
async function saveNewLoan() {
  ...
  const loan = {
    ...
    outstanding: outstanding || principal, emi,
    interestRate: rate || 0, tenure: tenure || 0,
    emisPaid: paid, emisRemaining: (tenure || 0) - paid,
    startDate, endDate, emiStartDate: startDate, paymentHistory: []
  };
  await Storage.saveLoan(loan);
